import { getAvailableItemsAction, getItemsAction } from "@/app/action/borrowing/action";
                ...
                <div className="lg:col-span-7 flex flex-col gap-8">
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
                        {allItems.length === 0 ? (
                            ...
                            allItems.map(item => (
                                <div key={item.id} className="group relative flex flex-col rounded-2xl border border-gray-200 bg-white p-5 shadow-sm hover:shadow-md transition-shadow">
                                    <div className="mb-3 flex justify-between items-start">
                                        <span className="inline-flex items-center rounded bg-gray-100 px-2 py-1 text-[10px] font-semibold text-gray-600 uppercase tracking-wider">
                                            {item.category || "Umum"}
                                        </span>
                                        {item.available > 0 ? (
                                    ...
                                    <h3 className="font-semibold text-gray-900 mb-1.5 text-lg group-hover:text-blue-600 transition-colors">{item.name}</h3>

                                    <p className="text-sm text-gray-500 mb-6 flex-1">{item.description || "Tidak ada deskripsi."}</p>

                                    <div className="mt-auto border-t border-gray-100 pt-4 flex items-center justify-between text-sm">
                                        <span className="text-gray-500 font-medium">Sisa Stok</span>
                                        <span className={`font-bold ${item.available > 0 ? 'text-gray-900' : 'text-red-500'}`}>
                                            {item.available} <span className="text-gray-400 font-normal">/ {item.quantity} unit</span>
                                        </span>
                                    </div>
                                </div>
                            ))
